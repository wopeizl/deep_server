...
namespace deep_server {
    caffe_processor::caffe_processor(actor_config& cfg,
        ...
        preprocess_.assign(
            ...
            become(processor_);
            send(cf_manager, deep_manager_process_atom::value, backend_mode_t::caffe, this, datapointer);
        }
        );
            ...
            return (
                ...
                if (method == "flip") {
                    ...
                    if (!cvprocess::readImage(idata, pcaffe_data->cv_image)) {
                        fault("caffe Fail to read image£¡");
                        return;
                    }
                    double elapsed = std::chrono::duration_cast<mill_second_> (clock_::now() - beg_).count();
                    ...
                    if (!verifyCV_Image(pcaffe_data->cv_image, 0xffff, 0xffff, 3)) {
