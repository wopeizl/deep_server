    ...
    behavior http_worker(http_broker* self, connection_handle hdl, actor cf_manager) {
        ...
        return {
                [=](const new_data_msg& msg) {
                        ...
                        if(self->state.analyzer.isDone()) { 
                            if (m.method == http_method::HTTP_POST ){
                                ...
                                if (!reader.parse(m.body, value) || value.isNull() || !value.isObject()) {
                                ...
                                if (method == "flip") {
                                    ...
                                    if (getlibmode().tensorflow) {
                                        self->state.actor_processor = self->spawn<tf_processor>("http_processor", self, msg.handle, cf_manager);
                                        self->monitor(self->state.actor_processor);
                                        self->link_to(self->state.actor_processor);
                                        self->send(self->state.actor_processor, input_atom::value, m.body);
                                    }
                ...
                [=](connection_handle& handle, output_atom, int resDataType, std::string callback, http_output out) {
                    ...
                    else if (resDataType == TF_DETECT_RESULT) {
                        root["tf_detect_result"] = Json::Value(Json::arrayValue);
                        for (int i = 0; i < out.tf_detect_result.size(); ++i) {
                            Json::Value sub;
                            sub["x"] = out.tf_detect_result[i].x();
                            sub["y"] = out.tf_detect_result[i].y();
                            sub["w"] = out.tf_detect_result[i].w();
                            sub["h"] = out.tf_detect_result[i].h();
                            sub["class"] = out.tf_detect_result[i].class_();
                            sub["score"] = out.tf_detect_result[i].score();

                            root["tf_detect_result"][i] = sub;
                        }
                    }
                    else if (resDataType == TF_CLASSIFY_RESULT) {
                        root["tf_classify_result"] = Json::Value(Json::arrayValue);
                        for (int i = 0; i < out.tf_classify_result.size(); ++i) {
                            Json::Value sub;
                            sub["class"] = out.tf_classify_result[i].class_();
                            sub["score"] = out.tf_classify_result[i].score();

                            root["tf_classify_result"][i] = sub;
                        }
                    }
