...
# Usage:
#   deep_list_unique(<list_variable> [<list_variable>] [...])
macro(deep_list_unique)
  foreach(__lst ${ARGN})
    if(${__lst})
...
# Usage:
#   deep_clear_vars(<variables_list>)
macro(deep_clear_vars)
  foreach(_var ${ARGN})
    unset(${_var})
  endforeach()
endmacro()
...
# Usage:
#   deep_string_unique(<string_variable>)
function(deep_string_unique __string)
  if(${__string})
    set(__list ${${__string}})
...
# Usage:
#   deep_print_list(<list>)
function(deep_print_list)
  foreach(e ${ARGN})
    message(STATUS ${e})
  endforeach()
endfunction()
...
# Usage:
#   deep_merge_flag_lists(out_variable <list1> [<list2>] [<list3>] ...)
function(deep_merge_flag_lists out_var)
  set(__result "")
  foreach(__list ${ARGN})
...
# Usage:
#   deep_convert_absolute_paths(<list_variable>)
function(deep_convert_absolute_paths variable)
  set(__dlist "")
...
# Usage:
#   deep_parse_header(<file> <define1> <define2> <define3> ..)
macro(deep_parse_header FILENAME FILE_VAR)
  set(vars_regex "")
...
# Usage:
#   deep_parse_header_single_define(<library_name> <file> <define_name>)
function(deep_parse_header_single_define LIBNAME HDR_PATH VARNAME)
  set(${LIBNAME}_H "")
  if(EXISTS "${HDR_PATH}")
...
# Usage:
#   deep_option(<option_variable> "doc string" <initial value or boolean expression> [IF <condition>])
function(deep_option variable description value)
  set(__value ${value})
  set(__condition "")
...
# Usage:
#   deep_compare_lists(<list_variable> <list2_variable> [description])
function(deep_compare_lists list1 list2 desc)
  set(__list1 ${${list1}})
  set(__list2 ${${list2}})
  list(SORT __list1)
  list(SORT __list2)
...
# Usage:
#   deep_warnings_disable(<CMAKE_[C|CXX]_FLAGS[_CONFIGURATION]> -Wshadow /wd4996 ..,)
macro(deep_warnings_disable)
  set(_flag_vars "")
  set(_msvc_warnings "")
  set(_gxx_warnings "")
  ...
  deep_clear_vars(_flag_vars _msvc_warnings _gxx_warnings)
endmacro()

################################################################################################
# Helper function get current definitions
# Usage:
#   deep_get_current_definitions(<definitions_variable>)
function(deep_get_current_definitions definitions_var)
...
# Usage:
#   deep_get_current_cflags(<cflagslist_variable>)
function(deep_get_current_cflags cflags_var)
  get_property(current_includes DIRECTORY PROPERTY INCLUDE_DIRECTORIES)
  deep_convert_absolute_paths(current_includes)
  deep_get_current_definitions(cflags)

  foreach(i ${current_includes})
    list(APPEND cflags "-I${i}")
  endforeach()

  deep_list_unique(cflags)
...
# Usage:
#   deep_parse_linker_libs(<Deep_LINKER_LIBS_var> <directories_var> <libflags_var> <frameworks_var>)
function(deep_parse_linker_libs Deep_LINKER_LIBS_variable folders_var flags_var frameworks_var)
  ...
  foreach(list_elem ${${Deep_LINKER_LIBS_variable}})
  ...
  deep_list_unique(libflags folders)
...
# Usage:
#   deep_detect_darwin_version(<version_variable>)
function(deep_detect_darwin_version output_var)
  if(APPLE)
